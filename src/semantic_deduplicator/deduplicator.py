...
class SemanticDeduplicator:
    def __init__(self, similarity_background="", final_similarity_threshold=0.8, cosine_similarity_threshold=.75, openai_api_key = ''):
        ...
        if similarity_background=="":
            warnings.warn("The 'similarity_background' variable is empty. It's recommended to provide context on your data for better results.")
    ...
    def transform_item_name(self, item):
        ...
        new_item_name = self.call_llm(system_prompt=system_prompt, human_prompt=human_propmt)
    ...
    def call_llm(self, system_prompt="You are a helpful assistant.", human_prompt="Hello!", function_schema=[], model="gpt-4-0613"):
        params = {
            "model": model,
            "messages": [
    ...
    def get_llm_similarity(self, item_1, item_2):
        ...
        llm_similarity = self.call_llm(system_prompt=system_prompt,
    ...
    def semantic_search(self, item: str) -> List[str]:
        ...
        cosine_similarities = [self.get_cosine_similarity(item, existing_item['formatted_name']) for existing_item in self.deduplicated_items_list]
